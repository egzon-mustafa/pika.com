/**
 * Crawl Gazeta Blic - Dedicated Gazeta Blic News Crawler
 * 
 * This function crawls specifically GazetaBlic.com and saves articles to Supabase.
...
import { GazetaBlicProvider } from "@/providers/gazeta-blic.ts";
import { logger } from "@/utils/logger.ts";

/**
 * Main crawler function for Gazeta Blic
 */
async function crawlGazetaBlic(options?: ScrapingOptions): Promise<CrawlerResult> {
  logger.starting("Gazeta Blic crawler starting");
  ...
  try {
    ...
    // Check if Gazeta Blic provider is active
    logger.info("Checking Gazeta Blic provider status");
    const isActive = await databaseService.checkProviderStatus(Provider.GAZETA_BLIC);
    
    if (!isActive) {
      logger.warn("Gazeta Blic provider is not active");
      return {
        totalArticlesFetched: 0,
        provider: Provider.GAZETA_BLIC,
        errors: ["Gazeta Blic provider is not active"],
      };
    }
    
    logger.info("Gazeta Blic provider is active, proceeding with crawling");
    
    const provider = new GazetaBlicProvider(options);
    ...
    // Scrape articles from Gazeta Blic
    const scrapedArticles = await provider.scrapeArticles();
    logger.scraping(`Gazeta Blic: Found ${scrapedArticles.length} articles`);
    ...
    logger.info(`Gazeta Blic: ${newArticles.length} new articles to save`);
    
    let savedCount = 0;
    if (newArticles.length > 0) {
      // Save new articles to database
      savedCount = await databaseService.saveArticles(newArticles);
      logger.success(`Gazeta Blic: Successfully saved ${savedCount}/${newArticles.length} articles`);
    } else {
      logger.info("Gazeta Blic: No new articles found - all articles already exist in database");
    }
    ...
    // Log summary
    logger.stats("Gazeta Blic Crawler Summary");
    ...
    logger.error("Gazeta Blic crawler failed", { error });
...
Deno.serve(async (req) => {
  logger.starting(`Crawl Gazeta Blic starting at ${new Date().toISOString()}`);
  ...
  try {
    ...
    const result = await crawlGazetaBlic(options);

    // Check if provider was not active
    if (result.errors && result.errors.includes("Gazeta Blic provider is not active")) {
      logger.warn("Gazeta Blic provider is not active, returning user message");
    ...
    logger.success("Gazeta Blic crawler completed successfully");
